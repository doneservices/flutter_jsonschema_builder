...
void main() {
  group('extractJsonFormSteps', () {
    test('a plain schema needs no ui schema: scalars get their own step, '
    ...
    test('ui:group merges flat fields onto one step at the first member\'s '
      ...
      ''';
      final steps = extractJsonFormSteps(
        parseSchema(testJsonSchema, uiSchema: uiSchema),
      );
  ...
  group('ui schema scoping and hardening', () {
    ...
    test('partial nested ui:order keeps unlisted fields after listed ones', () {
      ...
      final name = schema.properties!.whereType<SchemaObject>().first;
  ...
  group('stepped display mode', () {
    ...
    testWidgets('shows one step at a time with the object title as header '
    ...
    testWidgets('last step submits data shaped like the schema structure', (
      tester,
    ) async {
      dynamic savedData;
      await tester.pumpWidget(
    ...
    testWidgets('a dependency-inserted step joins the flow, leaves it again, '
      ...
      await tester.pumpWidget(
        buildTestApp(
          JsonForm(
            ...
            steppedConfig: const JsonFormSteppedConfig(
    ...
    testWidgets('selecting an enum value auto-advances a single-field step '
    ...
    testWidgets('keyboard dismisses on steps without text input and stays down '
